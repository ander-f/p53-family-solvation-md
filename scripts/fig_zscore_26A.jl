...
plot!(p53_hydration,
    ...
    color=c[1],
    xlabel="p53 Sequence",
    ylabel="Hydration Score (2.6 Å)",
    xticks=(1:5:length(res_labels_p53), string.(res_labels_p53[1:5:end])),
    xrotation=90,
    subplot=sp)
plot!(df_p53_blocks.mean_26A,
    ribbon=(df_p53_blocks.std_26A,),
...
plot!(p63_hydration,
    ...
    xrotation=90,
    subplot=sp)
plot!(df_p63_blocks.mean_26A,
    ribbon=(df_p63_blocks.std_26A,),
    label="Mean of blocks ± SD",
    color=c[2],
    ls=:dash,
    alpha=0.5,
...
plot!(p73_hydration,
    ...
    xrotation=90,
    subplot=sp)
plot!(df_p73_blocks.mean_26A,
    ribbon=(df_p73_blocks.std_26A,),
    label="Mean of blocks ± SD",
    color=c[3],
    ls=:dash,
    alpha=0.5,
...
sp=1
annotate!(196,0.022,L"\mathrm{A}",14, subplot=sp)
sp=2
annotate!(196,0.022,L"\mathrm{B}",14, subplot=sp)
sp=3
annotate!(196,0.022,L"\mathrm{C}",14, subplot=sp)
sp=4
annotate!(196,0.022,L"\mathrm{D}",14, subplot=sp)

plot!(size=(840,1260))

#savefig("$base_dir/figures/fig_trapz_compare_26A.pdf")
